...
$sql = "SELECT DISTINCT oa.*, u.verificado
                    ...
                    WHERE                       
                        oa.id IN (
						SELECT DISTINCT ea.id_oferta 
						FROM etiquetas_x_alquileres ea
						LEFT JOIN usuarios_x_etiquetas ue ON ea.id_etiqueta = ue.id_etiquetas
						WHERE ue.usuario_id = $id_usuario
						)
                        AND oa.estado = 1 
